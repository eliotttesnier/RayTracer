...
RayTracer::Factory::CylinderFactory::CylinderFactory(const Math::Point3D &position, const Math::Vector3D &rotation, const Math::Vector3D &scale, double radius, double height
):
    _radius(radius),
    _height(height),
    _position(position),
